// script.js – Phase 1 Complete with Fixed Dashboard & Hours UI
...
async function initDashboard() {
  ...
  const html = `
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
      <div class="bg-[var(--card-bg)] p-6 rounded-2xl border border-[var(--border-main)] shadow-sm">
        <div class="text-3xl font-black text-accent">${stats.totalBookings}</div>
        <div class="text-sm text-gray-500 mt-1">Total Bookings</div>
      </div>
      <div class="bg-[var(--card-bg)] p-6 rounded-2xl border border-[var(--border-main)] shadow-sm">
        <div class="text-3xl font-black text-accent">${stats.totalReviews}</div>
        <div class="text-sm text-gray-500 mt-1">Reviews</div>
      </div>
      <div class="bg-[var(--card-bg)] p-6 rounded-2xl border border-[var(--border-main)] shadow-sm">
        <div class="text-3xl font-black text-accent">${stats.totalQuotes}</div>
        <div class="text-sm text-gray-500 mt-1">Quote Requests</div>
      </div>
      <div class="bg-[var(--card-bg)] p-6 rounded-2xl border border-[var(--border-main)] shadow-sm">
        <div class="text-3xl font-black text-accent">${stats.profileViews}</div>
        <div class="text-sm text-gray-500 mt-1">Profile Views</div>
      </div>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
      <!-- Left Column: Bookings & Services -->
      <div>
        <h2 class="text-2xl font-bold mb-4">Recent Bookings</h2>
        <div class="space-y-3">
          ${bookings.map(b => `
            <div class="bg-[var(--card-bg)] p-4 rounded-xl border border-[var(--border-main)] flex justify-between items-center flex-wrap gap-2">
              <div>
                <div class="font-bold">${b.client_name}</div>
                <div class="text-sm text-gray-500">${b.service_name || 'Service'} – ${new Date(b.start_time).toLocaleString()}</div>
              </div>
              <select onchange="updateBookingStatus('${b.id}', this.value)" class="p-2 border rounded-lg bg-[var(--bg-main)] text-sm">
        ...
        <div id="servicesList" class="space-y-2 mb-4">
          ${services.map(s => `
            <div class="flex justify-between items-center p-3 bg-[var(--card-bg)] rounded-xl border border-[var(--border-main)]">
              <span><strong>${s.name}</strong> – N$${s.price} / ${s.duration_minutes}min</span>
              <button onclick="deleteService('${s.id}')" class="text-red-500 hover:text-red-700 text-sm">Delete</button>
            </div>
          `).join('')}
          ${services.length === 0 ? '<div class="text-gray-500 text-center py-4">No services added yet.</div>' : ''}
        </div>
        
        <form id="addServiceForm" class="bg-[var(--card-bg)] p-4 rounded-xl border border-[var(--border-main)] space-y-3">
          <h3 class="font-bold">Add New Service</h3>
          <input name="name" placeholder="Service name" required class="w-full p-2 border rounded-lg bg-[var(--bg-main)]">
          <input name="price" placeholder="Price (N$)" required class="w-full p-2 border rounded-lg bg-[var(--bg-main)]">
          <input name="duration_minutes" placeholder="Duration (minutes)" required class="w-full p-2 border rounded-lg bg-[var(--bg-main)]">
      ...
      <div>
        ...
        <form id="profileForm" class="bg-[var(--card-bg)] p-4 rounded-xl border border-[var(--border-main)] space-y-3">
          <input name="business_name" value="${profile.business_name || ''}" placeholder="Business Name" class="w-full p-2 border rounded-lg bg-[var(--bg-main)]">
          <textarea name="description" placeholder="Description" rows="3" class="w-full p-2 border rounded-lg bg-[var(--bg-main)]">${profile.description || ''}</textarea>
          <input name="category" value="${profile.category || ''}" placeholder="Category (e.g., Hair Salon)" class="w-full p-2 border rounded-lg bg-[var(--bg-main)]">
          <input name="address" value="${profile.address || ''}" placeholder="Address" class="w-full p-2 border rounded-lg bg-[var(--bg-main)]">
          <input name="whatsapp_number" value="${profile.whatsapp_number || ''}" placeholder="WhatsApp Number" class="w-full p-2 border rounded-lg bg-[var(--bg-main)]">
          <button type="submit" class="btn-primary w-full">Update Profile</button>
        </form>

        <h2 class="text-2xl font-bold mt-8 mb-4">Business Hours</h2>
        <form id="hoursForm" class="bg-[var(--card-bg)] p-4 rounded-xl border border-[var(--border-main)]">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ${days.map((dayName, day) => {
              const hour = hours.find(h => h.day_of_week === day) || {};
              return `
                <div class="hour-card p-3 border rounded-lg">
                  <div class="font-bold mb-2">${dayName}</div>
                  <div class="flex gap-2 items-center">
                    <input type="time" name="open_${day}" value="${hour.open_time || ''}" class="p-2 border rounded-lg bg-[var(--bg-main)] flex-1" ${hour.is_closed ? 'disabled' : ''}>
                    <span>–</span>
                    <input type="time" name="close_${day}" value="${hour.close_time || ''}" class="p-2 border rounded-lg bg-[var(--bg-main)] flex-1" ${hour.is_closed ? 'disabled' : ''}>
                  </div>
...
async function uploadLogo(file) {
  ...
  const res = await apiFetch('/api/upload/logo', { method: 'POST', body: formData });
  const data = await res.json();
...
async function uploadCover(file) {
  const formData = new FormData();
  formData.append('image', file);
  const res = await apiFetch('/api/upload/cover', { method: 'POST', body: formData });
  const data = await res.json();
...
async function uploadServiceImage(serviceId, file) {
  const formData = new FormData();
  formData.append('image', file);
  const res = await apiFetch(`/api/upload/service/${serviceId}`, { method: 'POST', body: formData });
  const data = await res.json();
  ...
  // Hours form – with proper null handling
...
// Helper function to enable/disable time inputs when "Closed" is checked
