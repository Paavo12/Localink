...
async function initBusinessPage() {
  ...
  let html = `
   ...
   // ========== LOCATION MAP (Leaflet if lat/lng exist, else iframe fallback) ==========
${b.lat && b.lng ? `
  <div class="mt-8">
...
` : (b.address ? `
  <div class="mt-8">
    ...
    <a href="https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(b.address)}" 
       target="_blank" 
       class="btn-secondary mt-4 inline-flex items-center gap-2">Get Directions</a>
  </div>
` : '')}
    ${b.address ? `
  <div class="mt-8">
...
// Initialize client view map if coordinates exist
if (b.lat && b.lng && document.getElementById('businessViewMap')) {
  // Check if Leaflet is loaded (it should be from business.html)
  if (typeof L !== 'undefined') {
    const map = L.map('businessViewMap').setView([b.lat, b.lng], 15);
