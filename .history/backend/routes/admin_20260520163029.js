...
// Add these functions inside the existing admin.js (after the stats endpoint)
// ... existing code ...

// Get data for admin charts (new)
router.get('/chart-data', async (req, res) => {
  const registrations = await pool.query(`
    SELECT DATE(created_at) as date, COUNT(*) FROM users WHERE created_at >= NOW() - INTERVAL '30 days' GROUP BY DATE(created_at)
  `);
  const categories = await pool.query(`
    SELECT category, COUNT(*) FROM provider_profiles GROUP BY category
  `);
  const tiers = await pool.query(`
    SELECT subscription_tier, COUNT(*) FROM provider_profiles GROUP BY subscription_tier
  `);
  const cities = await pool.query(`
    SELECT SPLIT_PART(address, ',', -1) as city, COUNT(*) FROM provider_profiles GROUP BY city
  `);
  res.json({ registrations: registrations.rows, categories: categories.rows, tiers: tiers.rows, cities: cities.rows });
});

// Activity feed (new)
router.get('/activity-feed', async (req, res) => {
  const providers = await pool.query(`SELECT 'provider' as type, business_name as name, created_at FROM provider_profiles ORDER BY created_at DESC LIMIT 5`);
  const bookings = await pool.query(`SELECT 'booking' as type, id, created_at FROM appointments ORDER BY created_at DESC LIMIT 5`);
  const reviews = await pool.query(`SELECT 'review' as type, comment, created_at FROM reviews ORDER BY created_at DESC LIMIT 5`);
  const feed = [...providers.rows, ...bookings.rows, ...reviews.rows].sort((a,b) => new Date(b.created_at) - new Date(a.created_at)).slice(0,10);
  res.json(feed);
});
