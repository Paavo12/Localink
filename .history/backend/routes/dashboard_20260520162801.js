...
router.get('/stats', authenticateToken, async (req, res) => {
  ...
  const profileViews = await pool.query('SELECT COUNT(*) FROM profile_views WHERE provider_id = $1', [providerId]);
  res.json({
...
router.put('/profile', authenticateToken, async (req, res) => {
  ...
  await pool.query(`
    UPDATE provider_profiles SET business_name=$1, description=$2, category=$3, address=$4, lat=$5, lng=$6, whatsapp_number=$7
    WHERE user_id=$8
  `, [business_name, description, category, address, lat, lng, whatsapp_number, req.user.id]);
  res.json({ message: 'Profile updated' });
});

// Services CRUD (already exists, but ensure full)
...
router.post('/services', authenticateToken, async (req, res) => {
  ...
  const result = await pool.query(
    `INSERT INTO services (provider_id, name, description, price, duration_minutes) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
...
router.post('/hours', authenticateToken, async (req, res) => {
  ...
  await pool.query(
    `INSERT INTO business_hours (provider_id, day_of_week, open_time, close_time, is_closed)
     VALUES ($1,$2,$3,$4,$5)
     ON CONFLICT (provider_id, day_of_week) DO UPDATE SET open_time=$3, close_time=$4, is_closed=$5`,
    [req.user.id, day_of_week, open_time, close_time, is_closed]
  );
