...
router.post('/logo', authenticateToken, upload.single('image'), async (req, res) => {
  ...
  if (old.rows[0]?.logo_url) await deleteCloudinaryImage(old.rows[0].logo_url);
...
router.post('/cover', authenticateToken, upload.single('image'), async (req, res) => {
  ...
  if (old.rows[0]?.cover_image_url) await deleteCloudinaryImage(old.rows[0].cover_image_url);
